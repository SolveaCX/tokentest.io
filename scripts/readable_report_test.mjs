...
await fs.writeFile(input, JSON.stringify({
  ...
  result: {
    ...
    risk: {
      ...
      p0_fail_count: 0,
      p1_fail_count: 1,
      gate_reason: "1 P1 failure(s)",
      p1_failures: [{ key: "channel_vision", name: "视觉输入通道", detail: "image failed" }],
    },
    pack_results: [{
...
assert.match(html, /TokenTest 分层评测解读报告/);
assert.match(html, /分类 Pack/);
assert.match(html, /评测维度/);
assert.match(html, /测试场景/);
assert.match(html, /<details class="pack"/);
assert.match(html, /<summary class="packSummary"/);
