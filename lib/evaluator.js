...
function scoreAuthenticityPack({ requestedModel, resolvedModel, data, content, response, usage, provider }) {
  ...
  const categories = [
    ...
    category("text_baseline", "文本通道基线", "partial", provider === "openai" || /gpt|vision|image/i.test(requestedModel) ? 65 : 60, 100, ["capability/text"], "text probe executed; vision/document capability is scored in the channel pack"),
