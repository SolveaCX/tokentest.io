...
const TRUNCATION_DERIVED_WEIGHT = 0.1;
const DIMENSION_DEFS = [
  ...
  },
  {
    ...
    key: "d2_model_core",
    name: "输出纪律与确定性任务",
    english_name: "Output Discipline & Deterministic Tasks",
    weight: 30,
...
function scoreProbe({ requestedModel, provider, response, latency_ms, traceRaw = false }) {
  ...
  const authenticity = buildAuthenticityConfidence(categories);
  const risk = riskGate(categories, rawScore);
  const score = risk.score;
  const compatibility = buildCompatibilitySummary(dimensions, risk, score);
  ...
  return {
    verdict: verdictForAuthenticity(authenticity, risk, score),
    score,
    raw_score: rawScore,
    risk,
    authenticity,
    ...
    summary: summaryFor(score, identityOk, structureOk, behaviorOk, usageOk, risk, authenticity, compatibility),
    pack_results,
    dimensions,
    dimension_coverage,
    categories,
...
function scoreChannelCapabilityPack(probes, usage) {
  ...
  const categories = [
    category("channel_tool_use", "工具调用通道", toolOk ? "pass" : (toolProbe.error ? "fail" : "partial"), toolOk ? 100 : (toolProbe.error ? 15 : 45), 100, ["channel/tool-use"], toolOk ? "forced function call returned valid JSON arguments" : channelDetail(toolProbe, "tool call was missing or arguments were not valid"), "p2", weakReminder()),
...
function scoreTokenIntegrityPack(probes, usage, options = {}) {
  ...
  const reasoningOnlyOutput = isReasoningOnlyVisibleOutput(outputProbe, outputUsage);
  ...
  const outputStatus = outputReasonable ? "pass" : (outputPartial || outputFiltered || reasoningOnlyOutput) ? "partial" : "fail";
  const outputScore = outputReasonable ? 100 : outputFiltered ? 60 : reasoningOnlyOutput ? 55 : outputPartial ? 60 : 20;
  const outputSeverity = outputFiltered || reasoningOnlyOutput ? "p2" : "p1";
  const outputDetail = outputFiltered
    ? `provider content_filter returned empty content; output_tokens=${outputUsage.output_tokens}`
    : reasoningOnlyOutput
      ? `reasoning-heavy visible output starvation; output_chars=${outputContent.length}; output_tokens=${outputUsage.output_tokens}; reasoning_tokens=${reasoningTokenCount(outputProbe)}`
  ...
  const categories = [
    ...
    category("token_output_reasonableness", "Output Token 合理性", outputStatus, outputScore, 100, ["usage/output-ratio"], outputDetail, outputSeverity, outputFiltered || reasoningOnlyOutput ? weakReminder() : {}),
    ...
    category("token_no_cache_sanity", "无 Cache 请求计量 sanity", noCacheOk ? "pass" : "fail", noCacheOk ? 100 : 20, 100, ["usage/no-cache"], noCacheOk ? "ordinary request has positive input tokens and no cache read/create tokens" : `ordinary request reported unexpected cache tokens=${noCacheTokens}`, "p2", weakReminder()),
  ];
...
function buildAuthenticityConfidence(categories) {
  const weights = {
    llm_fingerprint: 22,
    model_registry: 12,
    structure: 14,
    ...
    signature: 8,
    header_provenance: 6,
    auth_compatibility: 8,
    token_audit: 4,
    latency_success_rate: 2,
  };
  let score = 0;
  let total = 0;
  const evidence = [];
  for (const [key, weight] of Object.entries(weights)) {
    ...
    if (!item) continue;
    score += (Number(item.score) || 0) * weight;
    total += weight;
    evidence.push({ key, name: item.name, status: item.status, score: item.score, detail: item.detail });
  }
  const confidenceScore = total ? Math.round(score / total) : 0;
  const blocking = evidence.filter((item) => ["llm_fingerprint", "structure", "auth_compatibility"].includes(item.key) && item.status === "fail");
  const verdict = blocking.length
    ...
    : confidenceScore >= 85
      ...
      : confidenceScore >= 70
        ? "needs_review"
        : "suspicious";
  const summary = verdict === "high_confidence"
    ? "Identity, protocol, nonce, auth and usage evidence are consistent with the requested model endpoint."
    : verdict === "needs_review"
      ? "Core identity/protocol evidence is mixed; review before treating this as an official or no-downgrade endpoint."
      : "Core identity/protocol evidence is weak or contradictory; possible routing, downgrade or compatibility issue.";
  return {
...
function truncationDerivedReminder() {
  return { score_weight: TRUNCATION_DERIVED_WEIGHT };
}
...
function aggregateEndpointGenerationTruncation(packResults, probes) {
  ...
  const reasoningHeavy = truncationLooksReasoningHeavy(probes, truncatedProbeKeys);
  ...
  const endpointCategory = category(
    ...
    reasoningHeavy
      ? `${affectedP1Failures.length} high-risk failure(s) share reasoning-token-dominated length-limited evidence; treat as retest-needed endpoint compatibility evidence`
    ...
    reasoningHeavy ? "p2" : "p1",
    {
      affected_categories: affectedP1Failures.map((item) => item.key),
      affected_probes: Array.from(truncatedProbeKeys),
      ...(reasoningHeavy ? weakReminder() : {}),
    },
  );

  return packResults.map((packResult) => {
    const categories = (packResult.categories || []).map((item) => affectedKeys.has(item.key) ? downgradeTruncationDerivedCategory(item, { weakWeight: true }) : item);
...
function generationTruncatedProbeKeys(probes) {
  const expectedJsonProbeKeys = new Set([
    ...
    "public_truthfulqa",
    "public_code",
    "public_code_filter_reduce",
    "public_code_string_pipeline",
    "public_code_object_entries",
    "advanced_constraint",
    "advanced_table",
    "advanced_counterfactual",
    "advanced_proof",
    "safety",
  ]);
...
function truncationLooksReasoningHeavy(probes, keys) {
  const relevant = (probes || []).filter((probe) => keys.has(probe?.key));
  if (!relevant.length) return false;
  return relevant.some((probe) => {
    const usage = normalizeUsage(probe?.data?.usage || probe?.response?.usage || probe?.usage || {});
    return isLengthLimited(probe)
      && usage.output_tokens > 0
      && reasoningTokenCount(probe) >= usage.output_tokens * 0.75;
  });
}
...
function downgradeTruncationDerivedCategory(item, options = {}) {
  ...
  const weightMeta = options.weakWeight ? truncationDerivedReminder() : {};
  return {
    ...item,
    ...weightMeta,
...
function verdictForAuthenticity(authenticity, risk, score) {
  if (risk?.production_verdict === "blocked") return verdictFor(score);
  if (authenticity?.verdict === "high_confidence") return "genuine";
  if (authenticity?.verdict === "needs_review") return "suspicious";
  return verdictFor(score);
}

function buildCompatibilitySummary(dimensions, risk, score) {
  const weakDimensions = (dimensions || [])
    .filter((item) => Number(item.score) < 80)
    .map((item) => ({ id: item.id, key: item.key, name: item.name, score: item.score, status: item.status }));
  const focus = weakDimensions
    .sort((a, b) => Number(a.score) - Number(b.score))
    .slice(0, 3);
  const verdict = risk?.production_verdict || "unknown";
  const summary = verdict === "production_reference_pass"
    ? `Production compatibility score ${score}; review weaker dimensions before relying on strict structured-output workflows.`
    : `Production compatibility ${verdict}; ${risk?.gate_reason || "review reported gates and weak dimensions"}.`;
  return {
    score,
    verdict,
    summary,
...
function summaryFor(score, identityOk, structureOk, behaviorOk, usageOk, risk, authenticity, compatibility) {
  ...
  if (authenticity?.verdict === "high_confidence" && score < 80) {
    return `Authenticity high confidence (${authenticity.score}); production compatibility score ${compatibility?.score ?? score} needs review.`;
  }
