...
function scoreProbe({ requestedModel, provider, response, latency_ms, traceRaw = false }) {
  ...
  const pack_results = [
    ...
    scoreTokenIntegrityPack(response.probes, usage),
...
function scoreAuthenticityPack({ requestedModel, resolvedModel, data, content, response, provider }) {
  ...
  const nonceProbes = [byKey("authenticity"), byKey("protocol_nonce_2"), byKey("protocol_nonce_3")];
  const nonceCases = nonceProbes.map((probe, index) => {
    ...
    return testCase(`nonce_replay_call_${index + 1}`, `Nonce call ${index + 1}`, ok ? "pass" : "fail", ok ? 100 : 20, 100, ["protocol/nonce"], ok ? "current nonce echoed and no replay evidence" : `expected nonce ${probe?.nonce || "missing"} was not returned`, "p1", {
  ...
  const headerOk = modelProbe.http_status >= 200 && modelProbe.http_status < 300 && !headerLeak;
  ...
  const categories = [
    ...
    category("model_registry", "模型列表一致性", modelRegistryStatus, modelRegistryStatus === "pass" ? 100 : modelRegistryStatus === "partial" ? 55 : 20, 100, ["identity/models"], modelRegistryOk ? "requested model appears in /models or compatible alias list" : `model list evidence=${modelList.slice(0, 6).join(", ") || "unavailable"}`, "p1"),
    ...
    category("behavior", "行为验证", behaviorOk ? "pass" : "fail", behaviorOk ? 100 : 20, 100, ["behavior/json"], behaviorOk ? "probe JSON was returned" : "probe JSON was not returned", "p1"),
    categoryFromCases("nonce_replay", "Nonce 重放防护", nonceCases, ["protocol/nonce"], "all nonce probes echoed the current nonce", "one or more nonce probes missed the current nonce", "p1"),
    ...
    category("header_provenance", "响应 Header 溯源", headerOk ? "pass" : "fail", headerOk ? 100 : 20, 100, ["protocol/headers"], headerOk ? `header names=${Object.keys(modelProbe.headers || {}).join(", ") || "none"}` : "headers or /models response exposed internal or sensitive markers", "p1"),
...
function scoreReasoningPack(probe, publicCases = {}) {
  const parsed = extractJsonObject(probe);
  const arithmeticOk = Number(parsed?.arithmetic) === 30;
  ...
  const categories = [
    ...
    categoryFromCases("reasoning_code", "代码理解", codeCases, ["reasoning/code"], codeDetail, codeDetail, "p1", { gate_aggregate: true }),
  ];
...
function scoreChannelCapabilityPack(probes, usage) {
  ...
  const categories = [
    ...
    category("channel_vision", "视觉输入通道", visionOk ? "pass" : (visionProbe.error ? "fail" : "partial"), visionOk ? 100 : (visionProbe.error ? 15 : 45), 100, ["channel/vision"], visionOk ? "image input was accepted and answered correctly" : channelDetail(visionProbe, "vision probe did not return the expected JSON"), "p2"),
    ...
    category("channel_stream_sse", "流式 SSE 通道", streamOk ? "pass" : (streamProbe.error ? "fail" : "partial"), streamOk ? 100 : (streamProbe.error ? 20 : 50), 100, ["channel/stream-sse"], streamOk ? `SSE chunks=${streamProbe.stream.text_chunk_count}; TTFT=${streamProbe.stream.ttft_ms}ms` : channelDetail(streamProbe, "streaming SSE did not produce expected chunks or final [DONE]"), "p1"),
    category("channel_stream_delta", "流式 Delta 粒度", statusForScore(streamGranularityScore), streamGranularityScore, 100, ["channel/stream-delta"], streamRatio == null ? "stream usage tokens missing, delta granularity cannot be fully verified" : `delta_chunks/output_tokens=${streamRatio.toFixed(3)}`, "p2"),
...
function scoreTokenIntegrityPack(probes, usage) {
  const byKey = (key) => probes.find((item) => item.key === key) || {};
  ...
  const categories = [
    ...
    category("token_output_reasonableness", "Output Token 合理性", outputReasonable ? "pass" : outputPartial ? "partial" : "fail", outputReasonable ? 100 : outputPartial ? 60 : 20, 100, ["usage/output-ratio"], outputUsage.output_tokens > 0 ? `output_chars=${outputContent.length}; output_tokens=${outputUsage.output_tokens}; char/token=${charPerToken.toFixed(2)}` : "output token count missing", "p1"),
    ...
    category("token_stream_usage", "Stream Usage 一致性", streamUsageOk ? "pass" : "partial", streamUsageOk ? 100 : 50, 100, ["usage/stream"], streamUsageOk ? `stream usage input=${streamUsage.input_tokens} output=${streamUsage.output_tokens} total=${streamUsage.total_tokens}` : "stream completed but usage was missing or inconsistent", "p2"),
    category("token_cache_behavior", "Cache Token 双调用证据", statusForScore(cacheScore), cacheScore, 100, ["usage/cache-double-call"], cacheScore === 100 ? `cache_creation=${cacheCreation}; cache_read=${cacheRead}` : cacheProbeErrored ? "cache_control probe was rejected or failed; cache support not proven" : "cache_control probe succeeded but no cache creation/read usage was reported", "p2"),
...
function scorePerformancePack(probes) {
  ...
  const categories = [
    ...
    category("latency_ttft", "TTFT 首包延迟", statusForScore(ttftScore), ttftScore, 100, ["performance/ttft"], stream.ttft_ms != null ? `TTFT=${stream.ttft_ms}ms; chunks=${stream.text_chunk_count}` : "streaming TTFT was not observed", "p1"),
...
function scorePublicBenchmarkCases(probes) {
  ...
  const codeFilterReduce = extractJsonObject(byKey("public_code_filter_reduce"));
  const codeStringPipeline = extractJsonObject(byKey("public_code_string_pipeline"));
  const codeObjectEntries = extractJsonObject(byKey("public_code_object_entries"));
  ...
  return {
    ...
    reasoning_code: [
      testCase("code_benchmark_case", "Code benchmark-style case", codeOk ? "pass" : "fail", codeOk ? 100 : 20, 100, ["benchmark/code-style"], codeOk ? "code expression matched 3" : `expected 3, got ${code?.result ?? "missing"}`, "p2", {
        probe: "public_code",
        input: "公共代码理解题：计算 JS map/filter/reduce 表达式。",
        expected: "期望 result=3 且 tests=pass。",
      }),
      testCase("code_filter_reduce_case", "Filter/map/reduce case", codeFilterReduceOk ? "pass" : "fail", codeFilterReduceOk ? 100 : 20, 100, ["benchmark/code-style"], codeFilterReduceOk ? "even-square reduce expression matched 20" : `expected 20, got ${codeFilterReduce?.result ?? "missing"}`, "p2", {
        probe: "public_code_filter_reduce",
        input: "公共代码理解题：计算偶数平方后求和的 filter/map/reduce 表达式。",
        expected: "期望 result=20 且 tests=pass。",
      }),
      testCase("code_string_pipeline_case", "String pipeline case", codeStringPipelineOk ? "pass" : "fail", codeStringPipelineOk ? 100 : 20, 100, ["benchmark/code-style"], codeStringPipelineOk ? "string split/map/reduce expression matched 20" : `expected 20, got ${codeStringPipeline?.result ?? "missing"}`, "p2", {
        probe: "public_code_string_pipeline",
        input: "公共代码理解题：计算字符串 split 后长度乘积。",
        expected: "期望 result=20 且 tests=pass。",
      }),
      testCase("code_object_entries_case", "Object entries case", codeObjectEntriesOk ? "pass" : "fail", codeObjectEntriesOk ? 100 : 20, 100, ["benchmark/code-style"], codeObjectEntriesOk ? "Object.entries pipeline matched a2|b5" : `expected a2|b5, got ${codeObjectEntries?.result ?? "missing"}`, "p2", {
