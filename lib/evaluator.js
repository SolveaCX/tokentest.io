...
function scoreReasoningPack(probe, publicCases = {}) {
  ...
  const logicOk = String(parsed?.logic || "").replace(/\s+/g, "") === "Bo=Tue,Cy=Wed,Ana=Thu" || /Bo\s*=\s*Tue.*Cy\s*=\s*Wed.*Ana\s*=\s*Thu/is.test(content);
...
function scorePublicBenchmarkCases(probes) {
  ...
  const tableOk = Number(advancedTable?.refund_total) === 48 && Number(advancedTable?.restock_units) === 4 && String(advancedTable?.owner || "").toLowerCase() === "shared";
...
function advancedReasoningCategory(key, name, probe, ok, passDetail, failDetail, meta) {
  if (!probe?.key) {
  ...
  const filtered = isContentFiltered(probe);
  return category(key, name, ok ? "pass" : filtered ? "partial" : "fail", ok ? 100 : filtered ? 60 : 25, 100, ["reasoning/advanced"], ok ? passDetail : filtered ? "provider content_filter returned empty content for this advanced reasoning probe" : failDetail, filtered ? "p2" : "p1", { ...meta, ...(filtered ? weakReminder() : {}) });
}
